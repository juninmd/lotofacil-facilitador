...
export const generateConsensusGame = (history: LotofacilResult[], quantity: number = 15): number[] => {
    ...
    // XGBoost (2.2) - Improved GBDT
    addVote(generateXGBoostGame(history, quantity), 2.2);
...
export const simulateBacktest = async (fullHistory: LotofacilResult[], numSimulations: number = 20): Promise<SimulationResult> => {
    ...
    for (let i = 0; i < numSimulations; i++) {
        ...
        // Pattern Prediction (New)
        const patternPrediction = generatePatternGame(trainingData);
        const patternHits = patternPrediction.filter(n => targetGame.listaDezenas.includes(n)).length;
        ...
        // XGBoost Prediction (New)
        const xgbPrediction = generateXGBoostGame(trainingData);
        const xgbHits = xgbPrediction.filter(n => targetGame.listaDezenas.includes(n)).length;
