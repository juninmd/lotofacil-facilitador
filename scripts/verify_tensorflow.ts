...
import { generateTensorFlowGame } from '../src/utils/tensorflowStrategy';
...
async function runVerification() {
    console.log("=== TensorFlow Algorithm Verification ===");
    ...
    const candidate = await generateTensorFlowGame(history, 15);
    ...
    if (duration < 5000) {
        ...
        let totalHits = 0;
        for(let i=0; i<5; i++) {
             const c = await generateTensorFlowGame(history, 15);
